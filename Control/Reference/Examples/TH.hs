...
freeTypeVariables = fromTraversal (freeTypeVariables' [])
  where freeTypeVariables' bn f (ForallT vars ctx t) 
          = ForallT vars ctx <$> freeTypeVariables' (bn ++ (vars ^? traversal&typeVarName2)) f t
...
typeVarName = lens (\case PlainTV n _ -> n; KindedTV n _ k -> n) 
                   (\n' -> \case PlainTV _ s -> PlainTV n' s; KindedTV _ s k' -> KindedTV n' s k')
                   
-- | Reference the name of the type variable inside a type variable binder
typeVarName2 :: Simple Lens (TyVarBndr Specificity) Name
typeVarName2 = lens (\case PlainTV n _ -> n; KindedTV n _ k -> n) 
                   (\n' -> \case PlainTV _ s -> PlainTV n' s; KindedTV _ s k' -> KindedTV n' s k')
